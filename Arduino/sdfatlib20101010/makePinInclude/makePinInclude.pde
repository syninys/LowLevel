...
void printFile(Print* stream) {
  ...
  printType(stream);
  printLine(stream);
  println_P(stream, "#if defined(__AVR_ATmega1280__) || defined(__AVR_ATmega2560__)");
  ...
  printLine(stream);
  println_P(stream, "#else  // defined(__AVR_ATmega1280__) || defined(__AVR_ATmega2560__)");
  println_P(stream, "// 168 and 328 Arduinos");  
  printSection(stream, i2c328, spi328, pins328, nPin328);
  println_P(stream, "#endif  // defined(__AVR_ATmega1280__) || defined(__AVR_ATmega2560__)");
  printLine(stream);
...
//  printFile(&Serial);
  ...
  if (!root.openRoot(&vol)) error("root.openRoot");
  SdFile::remove(&root, "PIN_MAP.H");
  if (!file.open(&root, "PIN_MAP.H", O_WRITE |O_CREAT))error("file.open");
  printFile(&file);
  file.close();
